...
C = @zeros(size(a,2),ncomps,class(a));

normc = @(m)bsxfun(@rdivide,m,sqrt(sum(m.^2))); % returns normalized columns

w = ~isnan(a); % NAs have weight 0
a(~w) = 0;     % Original NA values changed to 0
%% empca
for comp = 1:ncomps
    ...
    for iter = 1:maxiters % repeat until u does not change or too many iterations
        ...
        Cw = bsxfun(@times,C(:,comp),w'); % M-step
        u(:,comp) = sum(a.*Cw',2)./(Cw'*C(:,comp)); % M-step
        u(:,comp) = normc(u(:,comp)); % M-step
        
        % simple M-step
        % u(:,comp) = normc(a*C(:,comp)); % M-step
    ...
    %%
    a = a - u(:,comp)*C(:,comp)'; % update A, removing converged component
    a(~w) = 0;
end
a(~w) = NaN; % Change missing values back to NA
s = diag(sqrt(sum(C.^2)));
v = normc(C);
