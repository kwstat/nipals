...
The `nipals` package provides two functions to perform Principal
Components Analysis of a matrix: 1. The `nipals` function uses
Non-linear Iterative Partial Least Squares. 2. The `empca` function uses
EM PCA.

NIPALS has been implemented several times in R packages. EMPCA has
previously appeared in python, but is available in R here for the first
time. This package strives to have the best (fast and accurate) R
implementations.

The [`empca()`](https://kwstat.github.io/nipals/reference/empca.md)
function should be considered **experimental**. There is a problem using
[`empca()`](https://kwstat.github.io/nipals/reference/empca.md) on
matrices that are both (1) non-full rank (2) have missing values.
...
A python version of this package can be found at
<https://pypi.org/project/nipals/>.
