---
title: "NIPALS performance notes"
author: "Kevin Wright"
...
These are some notes to document some of the development work for this package.
...
For testing purposes, a $100 \times 100$ matrix big enough so that tweaks to the code will show differences in performance time, but small enough so that each call of the function does not require a lot of waiting. A few missing values are inserted to force the code to use the method needed for missing data.
...
For the optimizing process, we use code taken from `mixOmics::nipals` since it avoids `for` loops over the columns of $X$, and should have better performance than `ade4::nipals` or `plsdepot::nipals`.

The timings are the median of 3 runs.
...
There's no need to keep the `ph.cross` object, and the `diag(crossprod())` is needlessly expensive since we only need the diagonal elements.
...
Most of the columns of `P` are the same, so `P*P` is repeating a lot of the same multiplications in the different columns. Better to square the numbers in one column, then put those into all columns. Also, there's no need to calculate `th` in two steps.
...
The first line of code is squaring the elements of `ph`, then outer multiplying by a vector of 1s to insert these into each column of `P2`. It makes sense algebraically, but we can avoid the multiplications and just build the matrix `P2` by repeating the columns.
...
Lest this look like an easy optimization, be aware there were numerous other versions, and each version often required fiddling with the syntax to make sure the right results were calculated.  For example, what happens if `ph` is a vector and is put into a matrix operation?  Not always what you might expect.
...
The total effect of all optimizations reduced the user time for the `nipals` function from 19.20 seconds to 3.38 seconds.

# Calculating PP' and TT'

In the Gram-Schmidt orthogonalization part of the algorithm, it is necessary to calculate $P_h P_h'$ where $P_h$ is a matrix of the first $h$ columns of the loadings matrix $P$.  It is not necessary to re-calculate the entire $P_h P_h'$ product for each PC, but only to update the product
$P_h P_h' = P_{h-1} P_{h-1}' + p_h p_h'$.  Here's a numerical verification of that:
...
```
```{r}
m1 = nipals(Bbig2, ncomp=10, gramschmidt=FALSE)
m2 = nipals(Bbig2, ncomp=10, gramschmidt=TRUE)
round(m1$eig,3)
round(m2$eig,3)
# P loadings
round(crossprod(m1$loadings),3)
round(crossprod(m2$loadings),3)
# T scores
round(crossprod(m1$scores),3)
