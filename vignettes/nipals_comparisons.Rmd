---
title: "Comparing NIPALS functions"
author: "Kevin Wright"
date: "October 27, 2017"
output: html_document
---

There are at least 4 R packages with a function for performing NIPALS on a matrix that contains missing values (and several other packages have functions which do not allow missing values).  The functions have slightly different scalings for the returned values, and were written with different coding styles.
...
The `ade4` package uses a scaling of the data which divides by `n` instead of `n-1`, so we need to scale the data by hand before using the `nipals` package.
...
mnip <- nipals::nipals(B2a, ncomp=5, center=FALSE, scale=FALSE, reconstruct=TRUE, maxiter=500, tol=1e-9)
```
...
library(nipals)
mnip <- nipals::nipals(B2a, ncomp=5, maxiter=100, tol=1e-6)
```
...
#   user  system elapsed 
#    0.5     0.0     0.5 
system.time(plsdepot::nipals(Bbig2, comps=100))
#   user  system elapsed 
#  30.19    0.00   30.18 
```
...
The `nipals::nipals` function was optimized through extensive testing and has the fastest execution time of all.

```{r, eval=FALSE}
system.time(nipals::nipals(Bbig2, ncomp=100)) # 100 factors
#   user  system elapsed 
#   2.74    0.00    2.75 
```
